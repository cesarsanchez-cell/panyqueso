...
import { uploadMyPhoto, type PhotoState } from "./photo-actions";
...
export function PhotoForm({ currentUrl, nombre }: Props) {
  const [state, formAction, pending] = useActionState<PhotoState, FormData>(uploadMyPhoto, null);
    ...
    <form action={formAction} className="flex flex-wrap items-center gap-4">
      <div className="h-20 w-20 shrink-0 overflow-hidden rounded-full bg-neutral-100 ring-1 ring-neutral-200">
        {shown ? (
          ...
          <img src={shown} alt="Tu foto de perfil" className="h-full w-full object-cover" />
        ) : (
